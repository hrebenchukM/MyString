...
public:
	...
	void operator() ();//Input.ѕерегрузка оператора преобразовани€ типа.возвращает другой тип например matrix(1,2)=4.5(матрица возвращает дабл) 
	...
	MyString& operator=(const MyString& b);//ѕерегрузка оператора =

	MyString(MyString&& b); //  онструктор переноса
	MyString& operator=(MyString&& obj); //ѕерегрузка оператора присваивани€ с переносом.
